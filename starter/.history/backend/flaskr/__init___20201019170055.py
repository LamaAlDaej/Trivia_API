...
def create_app(test_config=None):
  ...
  def search_questions():
    ...
    if search_term is None:
      # Send an error (resource not found - 404)
      abort(404)
    ...
    # Check if there are results or no
    if results is None:
      # If the results is empty, send an error (unprocessable - 422)
      abort(422)
  ...
  def get_category_questions(category_id):
    ...
    # Check if the category exists or not (if the user entered the URL with non-existent category ID)
    if category is None:
  ...
  """ This endpoint PLAY a quiz with specific category or all """
  # Set the method to POST
